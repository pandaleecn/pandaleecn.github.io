...
- 栈上 Block 使用 Copy 关键字时，会 Copy 到堆上，生成完全一样，内存空间不同的变量。当栈上变量作用域结束时销毁，堆上仍然存在，MRC 时会存在内存泄漏。
...
- [block 常见类型](https://segmentfault.com/a/1190000006479320)
  - \_NSConcreteGlobalBlock
    - 未捕获外部变量
    - 当需要布局（layout）的变量的数量为 0 时
  - \_NSConcreteStackBlock，默认
  - \_NSConcreteMallocBlock，\_NSConcreteStackBlock 在 MRC 复制时，ARC 默认从堆复制到栈上
- 多线程中 block 注意事项，\_\_strong NSObject \*strongObj = weakObj;
  ```
  //多线程时Block生命周期内对象安全
...
- 解决方案：\_\_weak NSArray \*weakArray = \_array;  
