...
- [UIKit](#uikit8899)
- [优化调试](#youhautiaoshi)
- [系统与架构](#xitongyujiagou)

### 1. 内存管理<a id="neicunguanli" ></a>

#### [关注内存问题，避免内存溢出和释放不及时](#关注内存问题，避免内存溢出和释放不及时)

#### [【腾讯真题】MLeakerFinder 有使用过吗？让你实现一个这样的框架怎么实现？](#MLeakerFinder有使用过吗)
...
#### [synchronized 和 dispatch_once](#synchronized和dispatch_once)

#### [OC 中的 volatile 关键字的作用是什么](#volatile关键字的作用是什么)
...
#### [C/C++的 static 关键字](#C/C++的static关键字)

#### [编译原理](#编译原理)

#### [C 语言中 extern 有哪些应用场景](#C 语言中 extern 有哪些应用场景)

#### [NSString 如何计算字符的个数](#NSString如何计算字符的个数)
...
### 9. 系统与架构<a id="xitongyujiagou" ></a>

#### [静态库与动态库](#静态库与动态库8888)

#### [CPU 的一级、二级缓存的作用是什么](#CPU 的一级、二级缓存的作用是什么)
...
- copy:表示赋值特性，setter 方法将传入的对象复制一份;需要完全一份新的变量时。
- nonatomic:非原子操作，决定编译器生成的 setter、getter 是否是原子操作。禁止多线程，变量保护，提高性能。
...
#### <a id="如何保障 App 的数据安全" >如何保障 App 的数据安全</a>
...
#### <a id="NSString如何计算字符的个数">NSString 如何计算字符的个数</a>

- NSString 是 UTF-16 编码的, 也就是 16 位的 unichar 字符的序列，一般遍历使用`[str characterAtIndex: i]`;
- Emoji 如“👍🏼”，用 2 个 16 位 unichar 来表示，它的 Unicode 是 U+1F44D, 用(U+D83D U+DC4D)两个字符来表示。遍历可使用`rangeOfComposedCharacterSequenceAtIndex`和`rangeOfComposedCharacterSequencesForRange`。

#### <a id="volatile关键字的作用是什么">volatile 关键字的作用是什么</a>
...
#### <a id="MLeakerFinder有使用过吗">【腾讯真题】MLeakerFinder 有使用过吗？让你实现一个这样的框架怎么实现？</a>
...
#### <a id="如何实现一个常驻线程">【腾讯真题】如何实现一个常驻线程</a>
...
#### <a id="关注内存问题，避免内存溢出和释放不及时">关注内存问题，避免内存溢出和释放不及时</a>
...
#### <a id="C/C++的static关键字">C/C++的 static 关键字</a>
...
#### <a id="CPU 的一级、二级缓存的作用是什么">CPU 的一级、二级缓存的作用是什么</a>
...
#### <a id="C 语言中 extern 有哪些应用场景">C 语言中 extern 有哪些应用场景</a>

- extern：外部声明用的。用来声明外部函数和变量定义。
- extern int total 表示 int total 这个变量在别的函数中被定义过。

#### <a id="synchronized和dispatch_once">synchronized 和 dispatch_once</a>
